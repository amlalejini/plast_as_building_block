...
def GenerateRandomlyChangingEnv(start = 50, stop = 200000, change_rate = 50):
    ...
    for trait in traits:
        ...
        ival2 = 0.5 * ival
        ival3 = 0.25 * ival
        env += "u 0 SetReactionValue %s %f \n" % (trait, ival)
        env += "u 0 SetReactionValue %s2 %f \n" % (trait, ival2)
        env += "u 0 SetReactionValue %s3 %f \n" % (trait, ival3)
    # Change env
    for u in range(start, stop + 1, change_rate):
        ...
        for trait in traits:
            ...
            ival2 = 0.5 * ival
            ival3 = 0.25 * ival
            env += "u %d SetReactionValue %s %f \n" % (u, trait, ival)
            env += "u %d SetReactionValue %s2 %f \n" % (u, trait, ival2)
            env += "u %d SetReactionValue %s3 %f \n" % (u, trait, ival3)
    return env
...
def main():
    ...
    #configs_dir = "/mnt/home/lalejini/exp_ws/plast_as_building_block/avida_configs/exp_configs_iter_3"
    configs_dir = "/Users/amlalejini/devo_ws/plast_as_building_block/avida_configs/exp_configs_iter_3"
